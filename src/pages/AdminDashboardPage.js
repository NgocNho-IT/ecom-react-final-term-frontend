...
const AdminDashboardPage = () => {
    ...
    const handleDeleteOrder = async (orderId) => {
        if (window.confirm("Nhớ chắc chắn muốn xóa đơn hàng này?")) {
            try {
    ...
    const handleDeleteProduct = async (productId, productName) => {
        if (window.confirm(`Nhớ muốn xóa sản phẩm ${productName}?`)) {
            try {
    ...
    const handleDeleteCategory = async (catId, catName) => {
        if (window.confirm(`Xóa danh mục ${catName} có thể làm mất sản phẩm bên trong, Nhớ chắc chưa?`)) {
            try {
                ...
                <div className="row">
                    ...
                    <div className="col-md-2 mb-4">
                        <div className="card border-0 shadow-sm rounded-4 p-3 sticky-top">
                            ...
                            <div className="nav flex-column nav-pills">
                                <button className={`nav-link mb-2 text-start ${activeTab === 'overview' ? 'active' : ''}`} onClick={() => setActiveTab('overview')}>
                                    <i className="bi bi-graph-up me-2"></i>Tổng quan
                                </button>
                                <button className={`nav-link mb-2 text-start ${activeTab === 'orders' ? 'active' : ''}`} onClick={() => setActiveTab('orders')}>
                                    <i className="bi bi-cart3 me-2"></i>Đơn hàng
                                </button>
                                <button className={`nav-link mb-2 text-start ${activeTab === 'products' ? 'active' : ''}`} onClick={() => setActiveTab('products')}>
                                    <i className="bi bi-box-seam me-2"></i>Sản phẩm
                                </button>
                                <button className={`nav-link mb-2 text-start ${activeTab === 'categories' ? 'active' : ''}`} onClick={() => setActiveTab('categories')}>
                                ...
                                <a href={`${BACKEND_URL}/api/admin/export-excel`} className="btn btn-outline-success rounded-pill btn-sm mb-2"><i className="bi bi-file-earmark-excel me-1"></i> Xuất Excel</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="col-md-10">
                        <div className="tab-content">
                            ...
                            {activeTab === 'orders' && (
                                <div className="tab-pane show active">
                                    <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
                                        <div className="card-header bg-white py-3"><h5 className="fw-bold mb-0">Quản Lý Đơn Hàng</h5></div>
                            ...
                            {/* TAB SẢN PHẨM - ĐÃ SỬA LINK ẢNH */}
                            {activeTab === 'products' && (
                                <div className="tab-pane show active">
                                    <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
                                        <div className="card-header bg-white py-3"><h5 className="fw-bold mb-0">Kho Sản Phẩm</h5></div>
                                        <div className="table-responsive">
                                            <table className="table table-hover align-middle mb-0">
                                                <thead className="table-light">
                                ...
                                <div className="tab-pane show active">
                                    <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
                                        <div className="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                                            ...
                                            <Link to="/admin/category/add" className="btn btn-sm btn-success rounded-pill px-3">+ Thêm mới</Link>
                                        </div>
                                        <div className="table-responsive">
                                            <table className="table table-hover align-middle mb-0">
                                                <thead className="table-light">
