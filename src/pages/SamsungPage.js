...
const SamsungPage = () => {
    ...
    useEffect(() => {
        ...
        if (saleProducts && saleProducts.length > 1) {
            slideInterval = setInterval(() => {
                const nextBtn = document.querySelector('#samsungCarousel .carousel-control-next');
    ...
    if (loading && products.length === 0) return (<div className="d-flex justify-content-center align-items-center" style={{ minHeight: '60vh' }}><div className="spinner-border text-primary"></div></div>);
        ...
        <>
            <style>
                {`
                    ...
                    :root { --brand-color: #1428A0; --brand-light: #2a41d0; }
                    ...
                    .product-card:hover { transform: translateY(-6px); box-shadow: 0 12px 24px rgba(20, 40, 160, 0.15) !important; border-color: var(--brand-color) !important; z-index: 2; }
                    ...
                    .badge-hot { background: linear-gradient(45deg, #1428A0, #415FFF); }
                    
                    .flat-banner-container { background: linear-gradient(120deg, var(--brand-color) 0%, var(--brand-light) 100%); border-radius: 16px; position: relative; margin-top: 1.5rem; margin-bottom: 2rem; box-shadow: 0 10px 30px rgba(20, 40, 160, 0.2); overflow: hidden;}
            ...
            <div className="container">
                <div id="samsungCarousel" className="carousel slide flat-banner-container" data-bs-ride="carousel">
                    <div className="bg-shape-1"></div><div className="bg-shape-2"></div>
                    <div className="carousel-indicators custom-indicators">
                        {saleProducts.map((_, index) => (
                            <button key={index} type="button" data-bs-target="#samsungCarousel" data-bs-slide-to={index} className={index === 0 ? "active" : ""}></button>
                        ))}
                    </div>
                                    ...
                                    <div key={p._id} className={`carousel-item ${index === 0 ? 'active' : ''}`}>
                                        <div className="row align-items-center" style={{ minHeight: '400px', padding: '2.5rem 4rem' }}>
                                            <div className="col-lg-7 z-1 mb-4 mb-lg-0 pe-lg-4">
                                                <div className="flat-banner-badge"><i className="bi bi-phone me-1"></i> Ưu đãi Samsung</div>
                                                <h1 className="flat-banner-title text-truncate">{p.name}</h1>
                                                <p className="flat-banner-desc">{p.description || "Đột phá công nghệ - Đỉnh cao di động. Khám phá Galaxy mới nhất với giá cực sốc."}</p>
                                                {v && (
                            ...
                            <div className="carousel-item active">
                                <div className="row align-items-center" style={{ minHeight: '400px', padding: '2.5rem 4rem' }}>
                                    <div className="col-lg-12 text-center z-1">
                                        <i className="bi bi-stars text-white mb-3" style={{fontSize: '4rem'}}></i>
                                        <h1 className="flat-banner-title">ĐỈNH CAO SAMSUNG</h1>
                                        <p className="flat-banner-desc text-center">Tiên phong công nghệ màn hình gập và AI.</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    {saleProducts.length > 1 && (
                        <>
                            <button className="carousel-control-prev d-none d-md-flex" type="button" data-bs-target="#samsungCarousel" data-bs-slide="prev">
                                <div className="custom-control-btn"><i className="bi bi-chevron-left text-white"></i></div>
                            </button>
                            <button className="carousel-control-next d-none d-md-flex" type="button" data-bs-target="#samsungCarousel" data-bs-slide="next">
            ...
            <section className="py-4 bg-light">
                <div className="container px-4 px-lg-5 mt-2">
                    <div className="d-flex justify-content-between align-items-end mb-4 border-start border-primary border-5 ps-3">
                        <h3 className="fw-bold text-dark mb-0 text-uppercase">{categoryName || "SAMSUNG GALAXY"}</h3>
                        <span className="text-muted small fw-bold bg-white px-3 py-1 rounded-pill shadow-sm border border-primary border-opacity-25">
                            Tổng cộng: <span className="text-primary">{totalItems}</span> sản phẩm
                        </span>
                    </div>
