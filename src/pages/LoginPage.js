...
const LoginPage = () => {
    ...
    // LOGIC VALIDATE MỚI (GIỮ LẠI ĐỂ CHẠY CHO CHUẨN)
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            email: Yup.string()
                .email('Địa chỉ email không hợp lệ (ví dụ: nho@gmail.com).')
    ...
    // Hàm lấy class cho border để đồng bộ với ban đầu
                                ...
                                <form onSubmit={formik.handleSubmit} noValidate>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <input 
                                            ...
                                            className={`form-control form-control-lg ${getFieldClass('email')}`}
                                            style={{ borderRadius: '12px', fontSize: '1rem' }}
                                            {...formik.getFieldProps('email')}
                                        />
                                        {formik.touched.email && formik.errors.email && (
                                            <div className="invalid-feedback fw-bold small mt-1">{formik.errors.email}</div>
                                        )}
                                    </div>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="input-group">
                                            <input 
                                                ...
                                                className={`form-control form-control-lg border-end-0 ${getFieldClass('password')}`}
                                                style={{ borderRadius: '12px 0 0 12px', fontSize: '1rem' }}
                                                {...formik.getFieldProps('password')}
                                            />
                                            <span 
                                                className={`input-group-text bg-white cursor-pointer ${formik.touched.password && formik.errors.password ? 'border-danger' : formik.touched.password && !formik.errors.password ? 'border-success' : ''}`}
                                            ...
                                            {formik.touched.password && formik.errors.password && (
                                                <div className="invalid-feedback fw-bold small mt-1" style={{ display: 'block' }}>{formik.errors.password}</div>
                                            )}
                                        </div>
                                    </div>

                                    <div className="d-grid gap-2 mt-4">
                                        <button type="submit" disabled={formik.isSubmitting} className="btn text-white shadow-sm" style={{ background: 'linear-gradient(45deg, #198754, #145a32)', padding: '12px', borderRadius: '50px', fontWeight: 'bold', fontSize: '1.1rem', border: 'none' }}>
                                            {formik.isSubmitting ? (
