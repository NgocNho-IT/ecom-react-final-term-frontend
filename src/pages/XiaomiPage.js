...
const XiaomiPage = () => {
    ...
    useEffect(() => {
        ...
        if (saleProducts && saleProducts.length > 1) {
            slideInterval = setInterval(() => {
                const nextBtn = document.querySelector('#xiaomiCarousel .carousel-control-next');
    ...
    if (loading && products.length === 0) return (<div className="d-flex justify-content-center align-items-center" style={{ minHeight: '60vh' }}><div className="spinner-border text-warning"></div></div>);
        ...
        <>
            <style>
                {`
                    ...
                    :root { --brand-color: #FF6900; --brand-light: #ff8c33; }
                    ...
                    .product-card:hover { transform: translateY(-6px); box-shadow: 0 12px 24px rgba(255, 105, 0, 0.15) !important; border-color: var(--brand-color) !important; z-index: 2; }
                    ...
                    .badge-hot { background: linear-gradient(45deg, #FF6900, #ff8c33); }
                    
                    .flat-banner-container { background: linear-gradient(120deg, var(--brand-color) 0%, var(--brand-light) 100%); border-radius: 16px; position: relative; margin-top: 1.5rem; margin-bottom: 2rem; box-shadow: 0 10px 30px rgba(255, 105, 0, 0.2); overflow: hidden;}
                    ...
                    .flat-banner-badge { background-color: #ffffff; color: #FF6900; font-size: 0.75rem; font-weight: 800; padding: 6px 14px; border-radius: 6px; display: inline-block; margin-bottom: 16px; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
            ...
            <div className="container">
                <div id="xiaomiCarousel" className="carousel slide flat-banner-container" data-bs-ride="carousel">
                    <div className="bg-shape-1"></div><div className="bg-shape-2"></div>
                    <div className="carousel-indicators custom-indicators">
                        {saleProducts.map((_, index) => (
                            <button key={index} type="button" data-bs-target="#xiaomiCarousel" data-bs-slide-to={index} className={index === 0 ? "active" : ""}></button>
                        ))}
                    </div>
                                    ...
                                    <div key={p._id} className={`carousel-item ${index === 0 ? 'active' : ''}`}>
                                        <div className="row align-items-center" style={{ minHeight: '400px', padding: '2.5rem 4rem' }}>
                                            <div className="col-lg-7 z-1 mb-4 mb-lg-0 pe-lg-4">
                                                <div className="flat-banner-badge"><i className="bi bi-lightning-fill me-1"></i> Flash Sale Xiaomi</div>
                                                <h1 className="flat-banner-title text-truncate">{p.name}</h1>
                                                <p className="flat-banner-desc">{p.description || "Cấu hình khủng - Mức giá siêu hời. Mua ngay kẻo lỡ."}</p>
                                                {v && (
                            ...
                            <div className="carousel-item active">
                                <div className="row align-items-center" style={{ minHeight: '400px', padding: '2.5rem 4rem' }}>
                                    <div className="col-lg-12 text-center z-1">
                                        <i className="bi bi-phone-vibrate text-white mb-3" style={{fontSize: '4rem'}}></i>
                                        <h1 className="flat-banner-title">ĐỘT PHÁ XIAOMI</h1>
                                        <p className="flat-banner-desc text-center">Ngon - Bổ - Rẻ. Vô địch phân khúc giá.</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    {saleProducts.length > 1 && (
                        <>
                            <button className="carousel-control-prev d-none d-md-flex" type="button" data-bs-target="#xiaomiCarousel" data-bs-slide="prev">
                                <div className="custom-control-btn"><i className="bi bi-chevron-left text-white"></i></div>
                            </button>
                            <button className="carousel-control-next d-none d-md-flex" type="button" data-bs-target="#xiaomiCarousel" data-bs-slide="next">
            ...
            <section className="py-4 bg-light">
                <div className="container px-4 px-lg-5 mt-2">
                    <div className="d-flex justify-content-between align-items-end mb-4 border-start border-warning border-5 ps-3">
                        <h3 className="fw-bold text-dark mb-0 text-uppercase">{categoryName || "XIAOMI"}</h3>
                        <span className="text-muted small fw-bold bg-white px-3 py-1 rounded-pill shadow-sm border border-warning border-opacity-25">
                            Tổng cộng: <span className="text-warning">{totalItems}</span> sản phẩm
                        </span>
                    </div>
